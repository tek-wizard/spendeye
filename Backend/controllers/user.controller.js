...
import { differenceInDays } from 'date-fns'; // THE FIX: Import date-fns for date comparison
...
export const handleGetContacts = async (req, res) => {
  try {
    ...
    return res.status(200).json({ // Use 200 for successful GET
      success: true,
...
export const handleAddContact = async (req, res) => {
  try {
    ...
    if (phoneNumber && user.contacts.find((contact) => contact.phoneNumber === phoneNumber)) {
      return res.status(400).json({
        success: false,
...
export const handleUpdateBudget = async(req,res) => {
  try {
    ...
    if (budget === undefined || isNaN(budget) || budget < 0) {
      return res.status(400).json({ success: false, message: "Invalid budget amount provided." });
    }
    ...
    if (user.budgetLastUpdated) {
      const daysSinceLastUpdate = differenceInDays(new Date(), user.budgetLastUpdated);
      if (daysSinceLastUpdate < 7) {
        return res.status(403).json({ // 403 Forbidden is a more appropriate status code
          success: false,
          message: `Budget can only be updated once per week. You can update it again in ${7 - daysSinceLastUpdate} days.`,
        });
      }
    }
