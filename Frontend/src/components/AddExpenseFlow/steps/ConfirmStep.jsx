...
export const ConfirmStep = ({ formData, onSubmit, isCreating, isEditMode }) => {
  ...
  const participants = useMemo(() => {
    ...
    const others = formData.contacts.map((contact) => ({
      ...contact,
      amount: (
        formData.splits.find((s) => s.participantId === contact._id)?.amount ||
        0
      ).toFixed(2),
    }))
    return [you, ...others]
  ...
  return (
    ...
    >
      ...
      >
        {isEditMode
          ? "Please review the details before saving the changes."
      ...
      >
        <CardContent>
          <Stack spacing={0} divider={<Divider flexItem />}>
            ...
            <DetailRow
              icon={
                <LocalOfferOutlinedIcon sx={{ color: iconColor, mt: "4px" }} />
              }
              label="Category"
            ...
            {formData.notes && (
              <DetailRow
                icon={
                  <NotesOutlinedIcon sx={{ color: iconColor, mt: "4px" }} />
                }
                label="Notes"
                      ...
                      >
                        <Avatar
                          sx={{
                            ...
                            height: 40,
                            bgcolor:
                              p.id === "user" ? "primary.main" : "accent.main",
                            color:
                              p.id === "user"
